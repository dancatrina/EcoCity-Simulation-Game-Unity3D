...
public class CategorySanatate : View , IPointerEnterHandler, IPointerExitHandler
{
    ...
    public UiBuildingInfoSanatate UiScriptInfo;
    private DetaliiSanatate containerFereastra;
    ...
    private void registerEvents()
    {
        foreach (var element in categories)
        {
            ...
            {
                UiBuildingInfoSanatate current = element.GetComponent<UiBuildingInfoSanatate>();
                if (current != null)
                {
                    ABuilding building = new BuildingSanatate(
                        current.numarMaximAngajati,
                        current.numarCurentAngajati,
                        current.nouNascutiTotal,
                        current.taxaCladire,
                        current.consumElectricitate,
                        current.venitCladire);
    ...
    {
        containerFereastra = detaliFereastra.GetComponent<DetaliiSanatate>();
        GameObject takeCurrentButton = eventData.pointerCurrentRaycast.gameObject;
        UiScriptInfo = takeCurrentButton.GetComponent<UiBuildingInfoSanatate>();
